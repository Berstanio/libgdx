...
public class DefaultIOSInput extends AbstractInput implements IOSInput {
	...
	private final UITextViewDelegate textViewDelegate = new UITextViewDelegate() {
		...
		public boolean textViewShouldChangeTextInRangeReplacementText (UITextView textView, NSRange range, String text) {
			...
			if (maxTextLength != null && textView.text().length() + (text.length() - range.length()) > maxTextLength) {
				return false;
			}
	...
	private final UITextFieldDelegate textDelegate = new UITextFieldDelegate() {
		...
		public boolean textFieldShouldChangeCharactersInRangeReplacementString (UITextField textField, NSRange range, String text) {
			...
			Integer cachedMaxTextLength = maxTextLength;
			if (cachedMaxTextLength != null && currentLength + (text.length() - range.length()) > cachedMaxTextLength) {
				return false;
			}
			InputStringValidator cachedInputStringValidator = inputStringValidator;
	...
	public void openTextInputField (final NativeInputConfiguration configuration) {
		...
		if (textfield instanceof UITextView) {
			...
			if (configuration.getType() == OnscreenKeyboardType.Password) {
				...
				button.addTargetActionForControlEvents(utilityCallback, new SEL("togglePasswordView"), UIControlEvents.TouchUpInside);
	...
	public void closeTextInputField (final boolean sendReturn) {
		...
		if (textfield instanceof UITextView) {
			...
			if (((UITextView)textfield).textColor().isEqual(UIColor.lightGrayColor())) text = "";
		} else {
			text = ((UITextField)textfield).text();
		}
		final long position = uiTextInput.offsetFromPositionToPosition(uiTextInput.beginningOfDocument(),
